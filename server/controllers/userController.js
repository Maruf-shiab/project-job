...
import { v2 as cloudinary } from "cloudinary";
import { clerkClient } from "@clerk/express";       // to fetch Clerk user details
...
export const getUserData = async (req, res) => {
  try {
    ...
    // Always fetch the latest Clerk profile
...
export const updateUserResume = async (req, res) => {
  try {
    ...
    const resumeFile = req.file;        // multer attaches file here
    if (!resumeFile) {
      return res.json({ success: false, message: "No resume file provided" });
    }

    // Allow non-image files (PDF/DOC) by using resource_type: "raw"
