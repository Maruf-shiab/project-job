...
const ViewApplications = () => {
  ...
  const fetchCompanyJobApllications = async () => {
    try {
            ...
            {applicants.filter(item => item.jobId && item.userId).map((applicant, index) => (
              <tr key={index} className='text-gray-700'>
                ...
                <td className='py-2 px-4 border-b'>
                  <a href={applicant.userId.resume} target='_blank' className='bg-blue-50 text-blue-600 px-3 py-1 rounded inline-flex gap-2 items-center '>Resume
                  ...
                  {applicant.status === "Pending"
                    ? <div className='relative inline-block text-left group'>
                      ...
                      <div className='z-10 hidden absolute right-0 md:left-0 top-0 mt-2 w-32 bg-white border-gray-200 rounded shadow group-hover:block '>
                        <button onClick={() => changeJobApplicationStatus(applicant._id, 'Accepted')} className='block w-full text-left px-4 oy-2 text-blue-500 hover:bg-gray-100'>ACCEPT</button>
                        <button onClick={() => changeJobApplicationStatus(applicant._id, 'Rejected')} className='block w-full text-left px-4 oy-2 text-red-500 hover:bg-gray-100'>REJECT</button>
                      </div>
                    </div>
